...
- `web_api_qwen.py`: 將 LLM 整合在 Web API 當中，接收請求，並生成回應。
- `gui_chat.py`: 支援拖曳上傳文件的 Tkinter 對話介面。
- `docs/`: 放置自訂知識庫文件（支援 .txt、.md、.pdf），呼叫 `/build` 後自動建立向量索引。
...
2. 建立與儲存 embeddings (包含對應的 passages，也就是 chunks)
    ```bash
    # 建立 emb.pkl 檔 (會在 test_dense_vector_search.py 和 web_api_qwen.py 使用)
    time python conv.py
